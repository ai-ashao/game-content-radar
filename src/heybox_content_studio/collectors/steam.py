            ...
            for row in list(rows or [])[:50]:
                ...
                if bucket == "specials":
                    ...
                    item.metrics.update({
                        ...
                        "historical_low_verified": False, "region": self.country, "store": "steam", "sku": str(appid), "edition": "app storefront snapshot",
                    })
                ...
                for row in ((data.get("appnews") or {}).get("newsitems") or []):
                    ...
                    result.items.append(
                        SourceItem(
                            ...
                            source_type=("official" if row.get("feedname") == "steam_community_announcements" else "media"),
                            game_name=game_name,
                            title=title,
                            summary=contents[:700],
                            published_at=published,
                            category=self._category(title, contents),
                            metrics={"appid": appid, "feedname": row.get("feedname"), "author": row.get("author"), "feedlabel": row.get("feedlabel")},
                            raw_text=contents,
                        )
                    )
